...
import data from '../data/irregular_verbs.json';
...
export function IrregularVerbsPage() {
    ...
    const rate = 0.7;
    return (
        <div className='irreg-verbs-container'>
            {experiences.map(data => (
                <div className='irreg-verbs-row'>
                    <TextToSpeech english={data["present"].split(" ")} past={data["past"]} participle={data["participle"]} meaning={data["meaning"].split("|")} en_voice={en_voice} es_voice={es_voice} rate={rate} />
                </div>
            ))}
        </div>
    )
